...
// Gerar o link de pagamento com Mercado Pago
function gerar_link_pagamento_mercado_pago($pedido, $vendedor_id) {
    ...
    // Obtendo as credenciais do vendedor
    $credentials_vendedor = get_mercado_pago_credentials(false); // Para o vendedor
    $access_token_vendedor = $credentials_vendedor['access_token']; // Token do vendedor
    ...
    $pedido = [
        ...
        'marketplace_fee' => 10, // Comissão do Marketplace
    ];
    ...
    $data = [
        ...
        'split' => [
            [
                'recipient_id' => $access_token_vendedor, // ID do vendedor obtido via OAuth
                'amount' => 68.18, // Valor que o vendedor vai receber após a comissão
                'application_fee' => 7.58 // Comissão do marketplace sobre o valor
            ],
            [
                'recipient_id' => $access_token_admin, // ID do marketplace
                'amount' => 7.58, // Comissão do marketplace
    ...
    if ($error) {
        ...
        if (isset($response_data['init_point'])) {
            ...
            return 'Acesse o link de pagamento: ' . $response_data['init_point'];
        } else {
            // Se não houver 'init_point' na resposta
            error_log('Erro ao gerar o link de pagamento: ' . print_r($response_data, true));
            return 'Erro ao gerar o link de pagamento. Verifique os logs para mais detalhes.';
        }
    }
}

// Exemplo de chamada da função para gerar o link de pagamento
echo gerar_link_pagamento_mercado_pago($pedido, $vendedor_id);
