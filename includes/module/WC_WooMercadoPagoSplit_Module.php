...
/**
 * Class WC_WooMercadoPagoSplit_Module
 */
class WC_WooMercadoPagoSplit_Module extends WC_WooMercadoPagoSplit_Configs
{
    public static array $categories = [];
    public static array $country_configs = [];
    public static $site_data;
    public static ?self $instance = null;
    public static array $mpInstancePayment = [];
    public static $mpInstance = null;
    public static $payments_name = null;
    public static array $notices = [];

    /**
     * WC_WooMercadoPagoSplit_Module constructor.
     */
    public function __construct()
    {
        try {
            ...
            add_filter('plugin_action_links_' . WC_MERCADOPAGO_SPLT_BASENAME, [$this, 'woomercadopago_settings_link']);
            add_filter('plugin_row_meta', [$this, 'mp_plugin_row_meta'], 10, 2);

            if (is_admin() && isset($_REQUEST['section'])) {
                $credentials = new WC_WooMercadoPagoSplit_Credentials();
                if (!$credentials->tokenIsValid()) {
                    add_action('admin_notices', [$this, 'enablePaymentNotice']);
                }
            }
        } catch (Exception $e) {
            $log = WC_WooMercadoPagoSplit_Log::init_mercado_pago_log('WC_WooMercadoPagoSplit_Module');
    ...
    {
        $credentials = new WC_WooMercadoPagoSplit_Credentials($payment);
        $validateCredentialsType = $credentials->validateCredentialsType();
        
        if ($validateCredentialsType === WC_WooMercadoPagoSplit_Credentials::TYPE_ACCESS_TOKEN) {
            $mp = new MPP($credentials->accessToken);
        } elseif ($validateCredentialsType === WC_WooMercadoPagoSplit_Credentials::TYPE_ACCESS_CLIENT) {
            $mp = new MPP($credentials->clientId, $credentials->clientSecret);
            if (!empty($payment)) {
    ...
    public static function getMpInstanceSingleton($payment = null)
    {
        if (!empty($payment)) {
            ...
            return self::$mpInstancePayment[$class] ?? null;
        }
    ...
    {
        include_once dirname(__FILE__) . '/../ payments/hooks/WC_WooMercadoPagoSplit_Hook_Abstract.php';
        include_once dirname(__FILE__) . '/../payments/hooks/WC_WooMercadoPagoSplit_Hook_Basic.php';
        include_once dirname(__FILE__) . '/../payments/hooks/WC_WooMercadoPagoSplit_Hook_Custom.php';
        include_once dirname(__FILE__) . '/../payments/hooks/WC_WooMercadoPagoSplit_Hook_Ticket.php';
    }
    ...
    {
        include_once dirname(__FILE__) . '/../helpers/WC_WooMercadoPagoSplit_Helpers_CurrencyConverter.php';
    }
    ...
    {
        include_once dirname(__FILE__) . '/preference/WC_WooMercadoPagoSplit_PreferenceAbstract.php';
        include_once dirname(__FILE__) . '/preference/WC_WooMercadoPagoSplit_PreferenceBasic.php';
        include_once dirname(__FILE__) . '/preference/WC_WooMercadoPagoSplit_PreferenceCustom.php';
        include_once dirname(__FILE__) . '/preference/WC_WooMercadoPagoSplit_PreferenceTicket.php';
        include_once dirname(__FILE__) . '/preference/analytics/WC_WooMercadoPagoSplit_PreferenceAnalytics.php';
    }
    ...
    {
        include_once dirname(__FILE__) . '/../payments/WC_WooMercadoPagoSplit_PaymentAbstract.php';
        include_once dirname(__FILE__) . '/../payments/WC_WooMercadoPagoSplit_BasicGateway.php';
    ...
    {
        include_once dirname(__FILE__) . '/../notification/WC_WooMercadoPagoSplit_Notification_Abstract.php';
        include_once dirname(__FILE__) . '/../notification/WC_WooMercadoPagoSplit_Notification_IPN.php';
        include_once dirname(__FILE__) . '/../notification/WC_WooMercadoPagoSplit_Notification_Webhook.php';
    }
    ...
    {
        include_once dirname(__FILE__) . '/log/WC_WooMercadoPagoSplit_Log.php';
    }
    ...
    public function loadAdminCss()
    {
        if (is_admin()) {
            ...
            wp_enqueue_style(
                'woocommerce-mercadopago-split-basic-config-styles',
    ...
    {
        include_once dirname(__FILE__) . '/../stock/WC_WooMercadoPagoSplit_Stock_Manager.php';
    }
    ...
    public function woomercadopago_settings_link($links)
    {
        ...
        $plugin_links = [
            '<a href="' . admin_url('admin.php?page=wc-settings&tab=checkout') . '">' . __('Set up', 'woocommerce-mercadopago-split') . '</a>',
        ];
    ...
    public static function define_link_country()
    {
        ...
        $sufix_country = 'AR';
        $country = [
            'AR' => ['help' => 'ayuda', 'sufix_url' => 'com.ar/', 'translate' => 'es'],
            'BR' => ['help' => 'ajuda', 'sufix_url' => 'com.br/', 'translate' => 'pt'],
            'CL' => ['help' => 'ayuda', 'sufix_url' => 'cl/', 'translate' => 'es'],
            'CO' => ['help' => 'ayuda', 'sufix_url' => 'com.co/', 'translate' => 'es'],
            'MX' => ['help' => 'ayuda', 'sufix_url' => 'com.mx/', 'translate' => 'es'],
            'PE' => ['help' => 'ayuda', 'sufix_url' => 'com.pe/', 'translate' => 'es'],
            'UY' => ['help' => 'ayuda', 'sufix_url' => 'com.uy/', 'translate' => 'es'],
        ];
        if ($wc_country !== '') {
        ...
        return array_key_exists($sufix_country, $country) ? $country[$sufix_country] : $country['AR'];
    }
    ...
    {
        if (WC_MERCADOPAGO_SPLT_BASENAME === $file) {
            return array_merge([$links[0]], [esc_html__('By Mercado Pago', 'woocommerce-mercadopago-split')]);
        }
        return (array)$links;
    }
    ...
    public static function is_valid_sponsor_id($sponsor_id)
    {
        ...
        if (empty($access_token) || $sponsor_id === '' || !is_numeric($sponsor_id)) {
        ...
        } elseif ($sponsor_id != get_option('_mp_sponsor_id', '') && $site_id != get_option('_mp_sponsor_site_id', '')) {
            $varify_sponsor = true;
        }

        if ($varify_sponsor) {
            $mp_sponsor_id = self::getMpInstanceSingleton();
            $get_sponsor_id = $mp_sponsor_id->get('/users/' . $sponsor_id, ['Authorization' => 'Bearer ' . $access_token], false);
            if (!is_wp_error($get_sponsor_id) && in_array($get_sponsor_id['status'], [200, 201])) {
                if ($get_sponsor_id['response']['site_id'] == $site_id) {
                    update_option('_mp_sponsor_id', $sponsor_id, true);
                    update_option('_mp_sponsor_site_id', $get_sponsor_id['response']['site_id'], true);
                } else {
    ...
    {
        return function_exists('WC') ? WC() : $GLOBALS['woocommerce'];
    }
    ...
    {
        $messages = [
            'Invalid payment_method_id' => __('The payment method is not valid or not available.', 'woocommerce-mercadopago-split'),
            'Invalid transaction_amount' => __('The transaction amount cannot be processed by Mercado Pago.', 'woocommerce-mercadopago-split') . ' ' .
                __('Possible causes: Currency not supported; Amounts below the minimum or above the maximum allowed.', 'woocommerce-mercadopago-split'),
            'Invalid users involved' => __('The users are not valid.', 'woocommerce-mercadopago-split') . ' ' .
                __('Possible causes: Buyer and seller have the same account in Mercado Pago; The transaction involving production and test users.', 'woocommerce-mercadopago-split'),
            'Unauthorized use of live credentials' => __('Unauthorized use of production credentials.', 'woocommerce-mercadopago-split') . ' ' .
                __('Possible causes: Use permission in use for the credential of the seller.', 'woocommerce-mercadopago-split'),
        ];
    ...
    public static function get_conversion_rate($used_currency)
    {
        ...
        return WC_WooMercadoPagoSplit_Helpers_CurrencyConverter::getInstance()->loadRatio($fromCurrency, $used_currency);
    }
    ...
    /**
     * Summary: Get information about the used Mercado Pago account based on its site.
     * Description: Get information about the used Mercado Pago account based on its site.
     * @return array|null
     */
    public static function get_site_data()
    {
        $site_id = get_option('_site_id_v1', '');
        return !empty($site_id) ? self::$country_configs[$site_id] : null;
    }
    ...
    {
        return get_woocommerce_currency() === self::$country_configs[$site_id]['currency'];
    }
    ...
    public static function build_currency_conversion_err_msg($currency)
    {
        return '<img width="14" height="14" src="' .
            ...
            __('ERROR: It was not possible to convert the unsupported currency', 'woocommerce-mercadopago-split') .
            ' ' . get_woocommerce_currency() . ' ' .
            __('to', 'woocommerce-mercadopago-split') . ' ' . $currency . '. ' .
            __('Currency conversions should be done outside of this module.', 'woocommerce-mercadopago-split');
    }
    ...
    public static function build_currency_not_converted_msg($currency, $country_name)
    {
        return '<img width="14" height="14" src="' .
            ...
            __('ATTENTION: The currency', 'woocommerce-mercadopago-split') .
            ' ' . get_woocommerce_currency() . ' ' .
            __('defined in WooCommerce is different from that used by the credentials of your country.<br>The currency for transactions made with this payment method will be', 'woocommerce-mercadopago-split') .
    ...
    public static function build_currency_converted_msg($currency)
    {
        return '<img width="14" height="14" src="' .
            ...
            __('CONVERTED CURRENCY: Your store is converting currency of', 'woocommerce-mercadopago-split') .
            ' ' . get_woocommerce_currency() . ' ' .
            __('for', 'woocommerce-mercadopago-split') . ' ' . $currency;
    }
    ...
    public static function get_country_name($site_id)
    {
        $countries = [
            ...
            'MLV' => __('Venezuela', 'woocommerce-mercadopago-split'),
    ...
    public static function get_map($selector_id)
    {
        ...
        foreach (wc_get_order_statuses() as $slug => $status) {
            ...
            $html .= sprintf(
                ...
                __('Update the WooCommerce order to ', 'woocommerce-mercadopago-split'),
                $status
            );
        }
    ...
    public static function is_product_dimensions_valid($all_product_data)
    {
        ...
        foreach ($all_product_data as $p) {
            ...
            if (!$product->is_virtual()) {
                ...
                if (empty($w) || !is_numeric($w) || !is_numeric($dimensions['height']) || !is_numeric($dimensions['width']) || !is_numeric($dimensions['length'])) {
                    return false;
                }
            }
        }
    ...
    {
        return version_compare(WC()->version, "4.0.0", ">");
    }
