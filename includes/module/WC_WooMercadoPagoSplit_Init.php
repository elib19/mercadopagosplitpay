...
class WC_WooMercadoPagoSplit_Init
{
    /**
    ...
    {
        $text_domain = 'woocommerce-mercadopago-split';
        $locale = apply_filters('plugin_locale', get_locale(), $text_domain);
        $original_language_file = dirname(__FILE__) . '/../../i18n/languages/woocommerce-mercadopago-split-' . $locale . '.mo';
    ...
    {
        $type = 'error';
        $message = esc_html__('Mercado Pago Split (WooCommerce + WCFM) requires PHP version 7.4 or later. Please update your PHP version.', 'woocommerce-mercadopago-split');
    ...
    {
        $type = 'error';
        $message = __('Mercado Pago Error: PHP Extension CURL is not installed.', 'woocommerce-mercadopago-split');
        echo WC_WooMercadoPagoSplit_Notices::getAlertFrame($message, $type);
    }

    /**
     * Notify user that WooCommerce is missing.
     */
    public static function notify_woocommerce_miss()
    {
        $type = 'error';
        $message = sprintf(
            __('The Mercado Pago module needs an active version of %s in order to work!', 'woocommerce-mercadopago-split'),
            ' <a href="https://wordpress.org/extend/plugins/woocommerce/">WooCommerce</a>'
        );
        echo WC_WooMercadoPagoSplit_Notices::getAlertWocommerceMiss($message, $type);
    }

    public static function add_mp_order_meta_box_actions($actions)
    {
        $actions['cancel_order'] = __('Cancel order', 'woocommerce-mercadopago-split');
        return $actions;
    }
    ...
    {
        if (!WC_WooMercadoPagoSplit_Module::isWcNewVersion() || (isset($_GET['page']) && $_GET['page'] === "wc-settings" && is_plugin_active('woocommerce-admin/woocommerce-admin.php'))) {
            return;
        }

        $noticesArray = WC_WooMercadoPagoSplit_Module::$notices;
    ...
    public static function woocommerce_mercadopago_init()
    {
        ...
        require_once dirname(__FILE__) . '../../admin/notices/WC_WooMercadoPagoSplit_Notices.php';
        WC_WooMercadoPagoSplit_Notices::initMercadopagoNotice();
        ...
        if (class_exists('WC_Payment_Gateway')) {
            require_once dirname(__FILE__) . '/config/WC_WooMercadoPagoSplit_Constants.php require_once dirname(__FILE__) . '/WC_WooMercadoPagoSplit_Exception.php';
            ...
            WC_WooMercadoPagoSplit_ReviewNotice::initMercadopagoReviewNotice();
